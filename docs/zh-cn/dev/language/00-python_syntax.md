...
> **该比较是语法匹配的，永不求值。** 在这个位置 `rc[0, 0] > 0` 是一份交给调度器的
> **声明式规格**（读哪个张量元素、做什么比较），**不是** `tensor.read` 加一次比较。
> 在 orchestration 里读这个值,正是谓词要消灭的事情。这也是只接受下述形状的原因。
...
镜像写法也被接受并归一化 —— `0 < rc[e]` 与 `rc[e] > 0` 记录同一个谓词（算子被翻转，
使 tensor 始终是操作数）。
